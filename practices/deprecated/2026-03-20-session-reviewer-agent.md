...
title: "session-reviewer agent as 7th standard agent in dotforge"
source_type: project-observation
source: dotforge (agents/session-reviewer.md)
status: deprecated
...
A `session-reviewer` agent was created during a real project session and committed to dotforge. It handles session analysis, pattern detection, and feeds `/forge insights`.

## Resolution

All evaluation questions resolved in v2.5.0:

1. **Generalizability**: Useful in any project with `/forge insights` or session analysis needs — not dotforge-specific.
