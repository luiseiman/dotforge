...
5. Write .claude/hooks/behaviors/_forge_runtime.sh
...
6. Update .claude/settings.json
   ...
   → idempotent: re-running produces identical output (see Section 6)

7. chmod +x all generated scripts
...
set -uo pipefail
# NOTE: set -e is intentionally omitted. grep returns exit 1 on no-match,
# which would kill the hook before remaining behaviors are evaluated.
# All error handling is explicit in _forge_runtime.sh functions.
...
|---|---|
| `regex_match` | `if echo "$val" \| grep -qE "$pattern"; then result=0; else result=1; fi` |
| `contains` | `if echo "$val" \| grep -qF "$pattern"; then result=0; else result=1; fi` |
| `not_contains` | `if echo "$val" \| grep -qF "$pattern"; then result=1; else result=0; fi` |
| `equals` | `if [[ "$val" == "$pattern" ]]; then result=0; else result=1; fi` |
| `starts_with` | `if [[ "$val" == "$pattern"* ]]; then result=0; else result=1; fi` |
| `ends_with` | `if [[ "$val" == *"$pattern" ]]; then result=0; else result=1; fi` |
| `gt` | `if [[ "$val" -gt "$pattern" ]]; then result=0; else result=1; fi` |
| `lt` | `if [[ "$val" -lt "$pattern" ]]; then result=0; else result=1; fi` |
| `gte` | `if [[ "$val" -ge "$pattern" ]]; then result=0; else result=1; fi` |
| `lte` | `if [[ "$val" -le "$pattern" ]]; then result=0; else result=1; fi` |
| `exists` | `if [[ -n "$val" ]]; then result=0; else result=1; fi` |
| `not_exists` | `if [[ -z "$val" ]]; then result=0; else result=1; fi` |

> All operators use explicit if/fi to avoid `set -e` issues with grep exit 1 on no-match.
> The `result` variable (0=match, 1=no match) is used by the composition logic.
...
## 6.1 Override Detection via Re-Invocation

Override audit is handled INSIDE the generated PreToolUse hook, not by a separate PermissionDenied hook.

> **Why not PermissionDenied?** Per Claude Code internals, `PermissionDenied` fires only on auto-mode classifier denials, NOT on PreToolUse hook denials. A behavior's `soft_block` emits `permissionDecision: "deny"` from PreToolUse — if the user overrides, Claude Code re-invokes the tool and PreToolUse fires again. See SPEC.md Section 6.2.

The generated PreToolUse hook includes an override detection preamble before behavior evaluation:

```bash
# --- Override detection (runs before behavior evaluation) ---
# Check if any behavior has a pending_block that matches this tool call
for bid in $(forge_get_pending_blocks "$SESSION_ID"); do
  pending_tool=$(forge_get_pending_block_field "$SESSION_ID" "$bid" "tool_name")
  pending_ts=$(forge_get_pending_block_field "$SESSION_ID" "$bid" "timestamp")

  # Match: same tool AND within 30-second window
  if [ "$pending_tool" = "$TOOL_NAME" ] && forge_within_window "$pending_ts" 30; then
    # This is an override — record audit trail
    forge_record_override "$SESSION_ID" "$bid" "$TOOL_NAME" "$TOOL_INPUT"
    forge_clear_pending_block "$SESSION_ID" "$bid"
    # Allow the tool call — do NOT re-evaluate triggers for this behavior
    forge_skip_behavior "$bid"
  else
    # Stale pending_block — clear it
    forge_clear_pending_block "$SESSION_ID" "$bid"
  fi
done
```

When a behavior emits `soft_block`, the hook also writes a `pending_block` to state.json:

```bash
# Inside evaluate_behavior(), when effective_level == soft_block:
forge_set_pending_block "$SESSION_ID" "$BEHAVIOR_ID" "$TOOL_NAME"
```
...
2. Adds entry to `overrides[]` in `.forge/runtime/state.json` (RUNTIME.md Section 2)
...
| bash 3.2+ | yes | all | Use bash 3.2-compatible constructs. macOS ships bash 3.2 — avoid bash 4+ features: `declare -A`, `${var,,}`, `mapfile`, `readarray`, `|&`, `coproc`. `[[ ]]` is fine (available since bash 2.02). `grep -P` (PCRE) is NOT available on macOS — use `grep -E` (ERE). |
