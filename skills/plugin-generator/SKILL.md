...
Generate a distributable Claude Code plugin from the current project's claude-kit configuration. The output is a standalone directory ready for `claude --plugin-dir` testing or marketplace submission.

## Input

`$ARGUMENTS` may contain:
- Output directory path (default: `./claude-kit-plugin/`)
- `--name <name>` to override plugin name (default: project directory name)

## Step 1: Validate source

Verify the current project has claude-kit configuration:
- `CLAUDE.md` must exist
- `.claude/settings.json` must exist
- `.claude/rules/` should exist (warn if missing)

If missing, error: "No claude-kit configuration found. Run `/forge bootstrap` first."
...
```json
{
  "name": "{project-slug}",
  "version": "1.0.0",
  "description": "Claude Code plugin generated from {project-name} configuration by claude-kit",
  "author": {
    "name": "{git user.name or 'Unknown'}"
  },
  "repository": "{git remote origin url or ''}",
  "license": "{detected license or 'MIT'}",
  "keywords": ["claude-code", "{stack1}", "{stack2}", "claude-kit"]
}
```
...
Claude Code plugin generated by [claude-kit](https://github.com/luiseiman/claude-kit).
