...
When syncing a project from an old dotforge version to a new one:
...
5. Manifest files dict does NOT need to be updated entry-by-entry during sync — only update `dotforge_version`, `last_sync`, `synced_at`.
